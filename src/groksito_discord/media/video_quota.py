"""Per-user daily video quota + one-at-a-time lock.

Persists to the bot data dir so Railway volume keeps counts across restarts.

Env:
  VIDEO_DAILY_LIMIT          default 5
  VIDEO_UNLIMITED_USER_IDS   comma-separated Discord snowflakes (creator)
  GROKSITO_OWNER_ID          also treated as unlimited
"""
...
def is_unlimited(user_id: int | str) -> bool:
    return str(user_id) in _unlimited_ids()


def used_today(user_id: int | str) -> int:
    uid = str(user_id)
    with _LOCK:
        data = _load()
...
def check_can_start(user_id: int | str, display_name: str = "") -> str | None:
    """Return a user-facing block message, or None if they may start."""
...
def mark_success(user_id: int | str, display_name: str = "") -> int:
    uid = str(user_id)
    with _LOCK:
        data = _load()
